 ...
 */
async function sendGamesTomorrowMessage(games) {
...
P.S. Ako želite biti dio kul ekipe koja dobije ping, provjerite jeste li podesili Discord username na https://odgrpg.org/ !`;
}
...
function describeOpenApplications(games) {
    const describedGames = games.map((game) => {
        return `- **${game.name}** (GM *${game.gm.name}*) - ${getDisplayDate(game.date)} u ${formatTime(game.date)}`;
    }).join('\n');

    const uniqueDates = [...new Set(games.map((game) => game.date.toDateString()))].sort();

    return `@everyone **Dobri narode, danas je velik i bitan dan dan!**

Na današnji dan u **18:30** otvaraju se prijave za vaš i naš **${games[0].odg}**, koji se igra od idućeg tjedna!
