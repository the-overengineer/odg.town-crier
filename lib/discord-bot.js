...
const { getISOWeek, isSameDay } = require('date-fns');
...
function extractNamedMembers(message) {
    const regex = /{{([^}]+)}}/g;
 ...
 * @param {import('./types').Game[]} games 
 */
async function sendEmptyGamesMessage(games) {
...
async function sendRawMessage(message) {
    const members = extractNamedMembers(message);
    console.log(JSON.stringify(members, null, 2));
    const formattedMessage = await insertPlayerReferences(message, members);
    return await sendDiscordMessage(formattedMessage);
}
...
function describeEmptyGames(games) {
    const emptyGames = games.filter((game) => game.spots > 0);
    const describedEmptyGames = emptyGames.map((game) => {
        const availableSpots = game.spots;
...
P.S. Ako želite biti dio kul ekipe koja sutra dobije ping u podsjetniku, provjerite jeste li podesili Discord username na https://odg.hr/ !`;
}
...
function describeOpenApplications(games) {
    ...
    const areAllOnTheSameDay = games.every((game) => isSameDay(game.date, games[0].date));

    const infix = areAllOnTheSameDay ? ' od' : '';

    return `@everyone **Dobri narode, danas je velik i bitan dan dan!**

Na današnji dan u **18:30** otvaraju se prijave za vaš i naš **${games[0].odg}**, koji se igra${infix} ${whichWeek} tjedna!
