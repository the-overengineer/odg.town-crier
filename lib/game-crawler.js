...
async function collectCardLinks(page) {
    ...
    // TODO: Remove
    await page.goto('https://odgrpg.org/odg/odg111');

    const cards = await page.$$('.view-content .card, .view-odg-event-sessions .card');
    const items = [];

    for (const card of cards) {
        const linkEl = await card.$('a');
        const nameEl = await card.$('.field--name-node-title')
        const odgEl = await card.$('.field--name-field-event a');
        ...
        const item = {
            ...
            odgLink: `${await odgEl.evaluate(el => el.href)}`,
...
async function enrichGameData(page, game) {
    ...
    const spotsEl = await page.$('.views-field-uid + div');

    const gmName = await gmEl.evaluate(el => el.textContent);
    const gmUrl = await gmEl.evaluate(el => el.href);
    const spots = await spotsEl.evaluate(el => el.textContent).then((text) => text.split(':')[1].trim()).then((text) => parseInt(text, 10));
